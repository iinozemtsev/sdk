...
DartEngine_SnapshotData AutoSnapshotFromFile(const char* path, char** error) {
  if (Dart_IsPrecompiledRuntime()) {
    return DartEngine_AotSnapshotFromFile(path, error);
  } else {
    return DartEngine_KernelFromFile(path, error);
  }
}
...
DART_EXPORT Dart_Timer Dart_Timer_Create(DartEngine_SnapshotData snapshot,
                                         char** error) {
  if (!DartEngine_Init(error) || *error != nullptr) {
    return nullptr;
  }
...
DART_EXPORT Dart_Timer Dart_Timer_CreateForPath(const char* path,
  ...
  DartEngine_SnapshotData snapshot = AutoSnapshotFromFile(path, error);
  if (*error != nullptr) {
...
DART_EXPORT int64_t Dart_Timer_GetTicks(Dart_Timer timer) {
  return WithIsolate<int64_t>(reinterpret_cast<Dart_Isolate>(timer), []() {
    return IntFromHandle(Dart_GetField(Dart_RootLibrary(),
